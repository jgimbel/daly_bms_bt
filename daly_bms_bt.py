...
def mqtt_iterator(result, base='', retain=False):
    name, id, time, val = result
    base = f'{name}-{id.replace(":", "-")}'
    topic = f'homeassistant/sensor/{base}'
    if not received_data:
        mqtt_config.config(mqtt_single_out, name, topic, val, base)
    mqtt_single_out(f'{topic}/state', json.dumps(val), retain)
...
async def main(con):
    ...
    while args.loop or not received_data:
        if not con.bt_bms.client.is_connected:
            await con.connect()
